...
├── desktop-version/    # Nova aplicação desktop integrada (Eel + HTML/JS UI)
├── electron-version/   # Versão antiga do aplicativo desktop (Node.js/Electron) [Legado]
├── local-version/      # Scripts de automação local e versão CLI via terminal
├── web-version/        # Solução legada baseada em aplicação Web (Flask) [Legado]
...
`pip install cloudscraper beautifulsoup4 tqdm requests eel`
...
2. Execute o comando: python .\app_desktop.py
